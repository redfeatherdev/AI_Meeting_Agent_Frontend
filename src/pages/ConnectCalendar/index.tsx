...
const ConnectCalendar = () => {
  ...
  const handleConnect = async () => {
    try {
      ...
      console.error("Failed to connect to Calendar due to ", err);
    }
  }

  const handleDelete = (email: string) => {
    axios.delete(`${import.meta.env.VITE_API_URL}/api/v1/google-calendar/delete-email/${email}/`, {
      headers: {
  ...
  useEffect(() => {
    axios.get(`${import.meta.env.VITE_API_URL}/api/v1/google-calendar/connected-emails/`, {
      headers: {
        'Content-Type': 'application/json'
      }
    })
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      {emails !== null && emails?.length > 0 && <span className="mt-5 text-lg text-black dark:text-white">Connected Emails</span>}
